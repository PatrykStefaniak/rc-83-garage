...
export const Navbar = () => {
    ...
    return (
        <nav className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-lg border-b border-border-light">
                    ...
                    <div className="hidden md:flex items-center space-x-1">
                        {navItems.map((item) => (
                            <a
                                key={item.href}
                                href={item.href}
                                className="px-4 py-2 rounded-lg text-foreground hover:bg-accent transition-smooth font-medium"
                            >
                                {item.label}
                            </a>
                        ))}
                    ...
                    <div className="md:hidden py-4 border-t border-border-light">
                        {navItems.map((item) => (
                            <a
                                key={item.href}
                                href={item.href}
                                className="block px-4 py-3 rounded-lg text-foreground hover:bg-accent transition-smooth font-medium"
